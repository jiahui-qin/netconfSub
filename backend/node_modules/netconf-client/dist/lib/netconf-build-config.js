...
export class NetconfBuildConfig {
    ...
    guessNamespace;
    get hasNamespace() {
        return this.namespace !== undefined && (typeof this.namespace === 'string' || (Array.isArray(this.namespace) && this.namespace.length > 0));
    }
    /**
     ...
     */
    constructor(xpath, schema, namespace, guessNamespace) {
        xpath = xpath.trim();
    ...
    build(targetObj) {
        if (!(this.xpath.includes('//') || this.xpath.includes('*'))) {
            ...
            return this.buildFromXPath(targetObj).pipe(switchMap(result => {
                if (result !== undefined) {
                    return of(result);
                }
    ...
    buildFromXPath(targetObj) {
        ...
        const namespace = this.hasNamespace
            ? of(this.getNamespaces())
            : this.guessNamespace
                ? of(null).pipe(tap(() => Output.debug('Sending request to the server to guess the namespace')), switchMap(() => this.guessNamespace ? this.guessNamespace : of(undefined)), map(ns => ns ? { xmlns: ns } : undefined))
                : of(undefined);
        return namespace.pipe(map(ns => {
     ...
     */
    buildFromSchema(schema, targetObj) {
        // remove leading / or //
        ...
        return schema.pipe(map((schemaObj) => {
        ...
        const searchSchema = (currentObj, steps, step) => {
            ...
            for (const key of Object.keys(currentObj)) {
                ...
                if (key === steps[0] || isWildcardMatch) {
                    ...
                    branchPassed = searchSchema(currentObj[key], newSteps.slice(1), step + 1) || branchPassed;
                }
                else if (steps[0] === '*' && steps.length === 1) {
                    branchPassed = searchSchema(currentObj[key], [], step + 1) || branchPassed;
                }
                else {
                    branchPassed = searchSchema(currentObj[key], steps, step + 1) || branchPassed;
                }
...
//# sourceMappingURL=netconf-build-config.js.map