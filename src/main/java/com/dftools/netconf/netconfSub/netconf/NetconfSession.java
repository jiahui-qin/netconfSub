...
public class NetconfSession {
    ...
    String getRpcReply(String rpc) throws IOException {
        ...
        synchronized (this) {
            ...
            log.trace("Received Netconf RPC-Reply\n{}", rpcReply);
     ...
     * @throws SAXException If there are issues parsing the config file.
     * @throws IOException  If there are issues reading the config file.
     */
     ...
     * @throws SAXException If the XML Reply cannot be parsed.
    ...
    public String myLockConfig() throws IOException, SAXException {
        String rpc = "<rpc>" +
                "<lock>" +
                "<target>" +
                "<candidate/>" +
        ...
        lastRpcReply = getRpcReply(rpc);
        if (hasError() || !isOK())
            throw new LoadException(lastRpcReply);
        return lastRpcReply;
    }

    public String discardChanges() throws IOException, SAXException {
        String rpc = "<rpc>\n" +
                "    <discard-changes/>\n" +
                "</rpc>" +
                NetconfConstants.DEVICE_PROMPT;
    ...
    public String myUnlockConfig() throws IOException, SAXException {
        String rpc = "<rpc>" +
                "<unlock>" +
                "<target>" +
                "<candidate/>" +
    ...
    public String setRunningConfiguration(String configuration) throws IOException, SAXException {
        String rpc = "<rpc>\n" +
                "  <edit-config>\n" +
                ...
                "      </config>\n" +
                "  </edit-config>\n" +
                "</rpc>" + NetconfConstants.DEVICE_PROMPT;
        lastRpcReply = getRpcReply(rpc);
        if (hasError() || !isOK())
            throw new LoadException("Load operation returned error");
        return lastRpcReply;
    ...
    public String setCandidateConfiguration(String configuration) throws IOException, SAXException {
        String rpc = "<rpc>\n" +
                "  <edit-config>\n" +
     ...
     * @throws IOException  if there are errors communication with the netconf server.
     * @throws SAXException if there are errors parsing the XML reply.
     */
     ...
     * @throws SAXException If there are errors parsing the XML reply.
     */
    public String commit() throws IOException, SAXException {
        String rpc = "<rpc>" +
                "<commit/>" +
                "</rpc>" +
                NetconfConstants.DEVICE_PROMPT;
        lastRpcReply = getRpcReply(rpc);
        if (hasError() || !isOK())
            throw new CommitException(lastRpcReply);
        return lastRpcReply;
    }
